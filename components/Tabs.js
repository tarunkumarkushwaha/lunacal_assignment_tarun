  ...
  return (
    <div className="bg-[#363C43] flex justify-center  rounded-2xl px-2 py-4 shadow-[0_0_25px_rgba(0,0,0,0.4)] w-[720px] h-[316px] mx-auto transition-all duration-300">
      ...
      <div className="px-2 flex flex-col justify-between w-[611px]">
        <div className="flex bg-[#0f1114] p-1 rounded-2xl mb-5 relative">
          {tabs.map((tab) => (
            <div key={tab} className="relative flex-1 group">
              <button
                onClick={() => setActive(tab)}
                className={`w-[195px] h-[49px] cursor-pointer rounded-2xl rotate-0 opacity-100 gap-2.5 pt-2.5 pr-6 pb-2.5 pl-6
                  font-poppins font-medium text-[18px] leading-[16.12px] text-center align-middle relative z-10 py-2 text-sm overflow-hidden transition-all duration-300 
               ${active === tab ? "text-white " : "text-gray-400"}`}>
                <span className="relative z-20">{tab}</span>
                {active !== tab && (
                  <div
                    className="absolute left-0 top-0 h-full bg-linear-to-r from-[#0f1114] to-[#272B30] w-0 transition-all duration-500 ease-in-out group-hover:w-full"
                  ></div>
                )}
              </button>
            </div>
          ...
          <motion.div
            className="absolute top-1 bottom-1 bg-[#28292F] rounded-2xl z-0"
            initial={false}
            animate={{
              left:
                active === "About Me"
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="font-jakarta overflow-x-hidden overflow-y-auto 
                ...
                [&::-webkit-scrollbar-track]:bg-[#363C43]
                [&::-webkit-scrollbar-thumb]:rounded-full
                [&::-webkit-scrollbar-thumb]:border-[3px]
                [&::-webkit-scrollbar-thumb]:border-solid
                [&::-webkit-scrollbar-thumb]:border-[#363C43]
                [&::-webkit-scrollbar-thumb]:bg-[linear-gradient(to_bottom,#555,#2b2b2b)]
                dark:[&::-webkit-scrollbar-thumb]:bg-[linear-gradient(to_bottom,#666,#333)]
               dark:[&::-webkit-scrollbar-track]:bg-[#363C43]"
            >
              <p className="mb-8">{content[active][0]}</p>
